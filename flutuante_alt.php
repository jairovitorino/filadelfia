...
<form name="form1" method="post" action="classes/interface.php" enctype="multipart/form-data" onSubmit="valida_dados(this)">
  <table width="80%" border="0" cellspacing="0" cellpadding="0" align="center">
	...
	$pk = $_SESSION["pk_flutuante_session"];
		
	?>
   <tr> 
      <td colspan="6" class="labelCentro" bgcolor="#0000FF"><div align="center"><font color="#FFFFFF">FLUTUANTE 
   ...
   echo "<div align='left'><img src='img/msg_verde.gif' width='20' height='20'><font color='#006600' size='2' face='Arial, Helvetica, sans-serif'> $msg </font></div>";
	...
	FROM flutuantes,status,profissoes,instrucao,entradas,estados, saidas, bairros
	WHERE flutuantes.fk_saida = saidas.pk_saida AND flutuantes.fk_estado = estados.pk_estado 
	AND flutuantes.fk_bairro = bairros.pk_bairro
	AND flutuantes.fk_entrada = entradas.pk_entrada AND flutuantes.fk_instrucao = instrucao.pk_instrucao 
	AND flutuantes.fk_profissao = profissoes.pk_profissao AND flutuantes.fk_sexo = status.pk_status AND pk_flutuante = ".$pk." ");
	//$sql = mysql_query("SELECT * FROM flutuantes,instrucao 
	//WHERE flutuantes.fk_instrucao = instrucao.pk_instrucao");
	$row = mysql_num_rows($sql);
	for ($z = 0; $z < $row; $z++){
	$id = mysql_result($sql, $z, "pk_flutuante");
	$te_foto = mysql_result($sql, $z, "te_foto");
	$dt_nascimento = mysql_result($sql, $z, "dt_nascimento");
	$fk_instrucao = mysql_result($sql, $z, "fk_instrucao");
	...
	<tr> 
      ...
      <td class="labelEsquerda"><input name="nm_flutuante" type="text" value="<?php echo mysql_result($sql, $z, "nm_flutuante");?>" size="50" maxlength="50"> 
      </td>
    ...
    <tr> 
      ...
      <td width="26%" class="labelEsquerda"><select name="fk_bairro" id="fk_bairro">
          ...
          <?php for($y=0; $y<$row_bairro; $y++) { ?>
          <option value="<?php echo mysql_result($sql_bairro, $y, "pk_bairro"); ?>"> 
          <?php echo mysql_result($sql_bairro, $y, "nm_bairro"); ?></option>
          <?php } ?>
        </select></td>
    </tr>
    <tr> 
    ...
    <tr> 
      ...
      <td class="labelEsquerda"> <?php
	   ...
	   <?php
		...
		$row_saida = mysql_num_rows($sql_saida);
		 ?>
	  <td class="labelDireita">Motivo</td>
    ...
    <tr> 
      <td class="labelEsquerda">&nbsp;</td>
      <td colspan="5"> <input type="hidden" name="alterarFlutuante" value="alterarFlutuante"> 
        <input type="hidden" name="pk_flutuante" value="<?php echo $id;?>">
